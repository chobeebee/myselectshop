    ...
    public ProductResponseDto updateProduct(Long id, ProductMypriceRequestDto requestDto) {
        ...
        if (myprice < MIN_MY_PRICE) {
            throw new IllegalArgumentException(
                    messageSource.getMessage(
                            "below.min.my.price", //code
                            new Integer[]{MIN_MY_PRICE}, //메세지 내에서 매개변수를 사용할 경우, 전달할 값 //{0} : 0번째 index값
                            "Wrong Price", //기본 문구
                            Locale.getDefault() //언어 설정 (국제화할때 사용할 수 있음,번역)
                    )
            );
        }
        ...
        Product product = productRepository.findById(id).orElseThrow(() ->
                new ProductNotFoundException(messageSource.getMessage(
                        "not.found.product", //code
                        null, //인접값 없음
                        " Not Found Product", //기본 문구
                        Locale.getDefault() //언어 설정 (국제화할때 사용할 수 있음,번역)
                ))
        );
