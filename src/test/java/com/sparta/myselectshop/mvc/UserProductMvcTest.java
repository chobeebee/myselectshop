...
@WebMvcTest(
        ...
        excludeFilters = {
                @ComponentScan.Filter(
                        ...
                        classes = WebSecurityConfig.class //직접 만든 config
                )
        }
)
class UserProductMvcTest {
    ...
    private Principal mockPrincipal; //가짜 Principal 필요 : 가짜 인증을 위해
    ...
    public void setup() {
        mvc = MockMvcBuilders.webAppContextSetup(context)
                .apply(springSecurity(new MockSpringSecurityFilter())) //직접 만든 가짜 필터 넣어주기 -> 가짜 Security Filter로 사용
                .build();
    }
    ...
    private void mockUserSetup() {
        ...
        User testUser = new User(username, password, email, role); //테스트할 User
